...
Concept links are outside the V2 automatic apply path. Use them only during explicit semantic review or with a separate retrieval workflow; routine Inbox classification should add MOC membership only.
...
- Never add a body relation section automatically.
- Create a hash-checked plan before apply.
- Verify the plan, then plan again to confirm idempotence.
